    ...
    return (
        <div>
            {notFound ? <p className="h-[80vh] flex justify-center items-center">{notFound}</p> 
                :
            <div>
                {donations.length>0 && <button onClick={handleRemove}  className="px-7 rounded btn btn-success mt-4 font-semibold text-sm py-1  border-2">Delete all favorites</button>}
         ...
         <button onClick={() => setIsShow(!isShow)} className="px-7 rounded btn  btn-success mt-4 ml-[120px] md:ml-[400px] xl:ml-[600px] font-semibold text-sm py-1  border-2">{isShow ? 'See Less' : 'See More'}</button>
         </div>
         }</div>
    );
};
